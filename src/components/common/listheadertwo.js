...
import { getCategorylist, gethomeList, getsearchList, likeList } from '../../services/action/list';
...
class ListHeaderTwo extends Component {
    ...
    fetchSearchlists = (obj) => {
        this.props.dispatch(getsearchList(obj)).then(() => {
            if (this.props.searchlists) {
                this.setState({
                    alllists: this.props.searchlists, mainlists: this.props.searchlists, loading: false
                })
                this.country()
                this.handleChangeCountryby({ value: "All", label: 'All' })
                                                                                    ...
                                                                                    <div className="card-item">
                                                                                        <Link to={`/listing-details/${item.listing.canonicalurl}`} className="card-image-wrap">
                                                                                            <div className="card-image">
                                                                                                <img src={item.listing.bannerimg ? `${process.env.REACT_APP_API_KEY}utilities/${item.listing.bannerimg}` : item.listing.listimage} className="card__img" alt={item.listing.list_title} />
...
function mapStateToProps(state) {
    ...
    const { lists, searchlists, categorylists } = state.list;
    const { amenties } = state.common;
    return {
        isLoggedIn, userdetails, lists, categorylists, amenties, searchlists
